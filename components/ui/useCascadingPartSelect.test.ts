...
describe("sortModelsByNumber — 같은 그룹 안 모델 번호 오름차순 정렬", () => {
  ...
  it("실제 버그: 뒤에 다이 코드네임(GA102, AD104)이 붙은 이름을 코드네임 숫자가 아니라 RTX/GTX 뒤 숫자로 정렬한다", () => {
    // /build의 브랜드→모델(시리즈 단계 없음) 화면에서 "GeForce RTX 3070 Ti 8 GB GA102"의
    // "102"를 모델 번호로 잘못 집어 목록 맨 앞에 튀어나오던 버그.
    const names = ["GeForce RTX 4060 Ti AD104", "GeForce GTX 960", "GeForce RTX 3070 Ti 8 GB GA102"];
    const sorted = sortModelsByNumber(names, (n) => n);
    expect(sorted).toEqual(["GeForce GTX 960", "GeForce RTX 3070 Ti 8 GB GA102", "GeForce RTX 4060 Ti AD104"]);
  });

  it("Intel Arc처럼 브랜드 접두(RTX/GTX/GT/RX)가 없는 GPU 이름은 여전히 모델 번호로 정렬된다", () => {
    const names = ["Intel Arc A770", "Intel Arc A380", "Intel Arc A750"];
    const sorted = sortModelsByNumber(names, (n) => n);
    expect(sorted).toEqual(["Intel Arc A380", "Intel Arc A750", "Intel Arc A770"]);
  });
